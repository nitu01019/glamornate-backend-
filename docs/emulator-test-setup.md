...
`backend/scripts/run-emulator-tests.sh` wrapper runs `npm install --workspaces=false` first to work
...
The script (`backend/scripts/run-emulator-tests.sh`) does three things:
