...
export const deleteAccount = callableOpts({
  ...
  .https.onCall(
    withRateLimit(
      // α8-1 (ε6, 2026-05-12): tightened from 30/min → 5/min, mirroring
      // revoke-sessions.ts F-DRACO-06 cap. Delete-account is irreversible;
      // 30/min was an unreasoned default. 5/min/uid covers legitimate
      // re-confirmation flows (confirm-string typos) and bounds the abuse
      // blast-radius on a compromised device.
      { name: 'deleteAccount', windowMs: 60_000, max: 5 },
